...
export class MessagesService {
	...
	getMessageListings(page: number = 1): Promise<MessageListing[]> {
		return this.http.get(`/walkoff/api/messages?page=${ page }`)
			.toPromise()
	...
	getMessage(messageId: number): Promise<Message> {
		return this.http.get(`/walkoff/api/messages/${messageId}`)
			.toPromise()
	...
	performActionOnMessages(messageIds: number | number[], action: string): Promise<void> {
		...
		return this.http.put('/walkoff/api/messages', { ids: messageIds, action })
			.toPromise()
			.then(() => null)
			.catch(this.utils.handleResponseError);
	}
	...
	respondToMessage(workflow_execution_id: string, action: string): Promise<string[]> {
		...
		return this.http.put('/walkoff/api/triggers/send_data', body)
			.toPromise()
			.catch(this.utils.handleResponseError);
	}
}
