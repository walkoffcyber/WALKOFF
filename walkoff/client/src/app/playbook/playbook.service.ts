...
export class PlaybookService {
	...
	newPlaybook(playbook: Playbook): Promise<Playbook> {
		return this.http.post('/walkoffapi/playbooks', classToPlain(playbook))
			.toPromise()
			.then((data) => plainToClass(Playbook, data))
			.catch(this.utils.handleResponseError);
	}
	...
	renamePlaybook(playbookId: string, newName: string): Promise<Playbook> {
		return this.http.patch('/walkoffapi/playbooks', { id: playbookId, name: newName })
			.toPromise()
	...
	duplicatePlaybook(playbookId: string, newName: string): Promise<Playbook> {
		return this.http.post(`/walkoffapi/playbooks?source=${playbookId}`, { name: newName })
			.toPromise()
			.then((data) => plainToClass(Playbook, data))
			.catch(this.utils.handleResponseError);
	}
	...
	deletePlaybook(playbookIdToDelete: string): Promise<void> {
		return this.http.delete(`/walkoffapi/playbooks/${playbookIdToDelete}`)
			.toPromise()
			.catch(this.utils.handleResponseError);
	}
	...
	exportPlaybook(playbookId: string): Observable<Blob> {
		return this.http.get(`/walkoffapi/playbooks/${playbookId}?mode=export`, { responseType: 'blob' })
			.catch(this.utils.handleResponseError);
	}
	...
	importPlaybook(fileToImport: File): Observable<Playbook> {
		...
		return this.http.post('/walkoffapi/playbooks', formData, { headers })
	...
	duplicateWorkflow(
		...
		return this.http.post(`/walkoffapi/workflows?source=${sourceWorkflowId}`,
	...
	deleteWorkflow(workflowIdToDelete: string): Promise<void> {
		return this.http.delete(`/walkoffapi/workflows/${workflowIdToDelete}`)
			.toPromise()
			.catch(this.utils.handleResponseError);
	}
	...
	newWorkflow(playbookId: string, workflow: Workflow): Promise<Workflow> {
		...
		return this.http.post('/walkoffapi/workflows', classToPlain(workflow))
			.toPromise()
			.then((data) => plainToClass(Workflow, data))
			.catch(this.utils.handleResponseError);
	}
	...
	saveWorkflow(workflow: Workflow): Promise<Workflow> {
		return this.http.put('/walkoffapi/workflows', classToPlain(workflow))
			.toPromise()
			.then((data) => plainToClass(Workflow, data))
			.catch(this.utils.handleResponseError);
	}
	...
	getApis(): Promise<AppApi[]> {
		return this.http.get('/walkoffapi/apps/apis')
			.toPromise()
