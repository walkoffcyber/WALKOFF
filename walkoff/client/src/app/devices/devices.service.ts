...
export class DevicesService {
	...
	getDevices(page: number = 1): Promise<Device[]> {
		return this.http.get(`/walkoff/api/devices?page=${ page }`)
			.toPromise()
	...
	addDevice(device: Device): Promise<Device> {
		return this.http.post('/walkoff/api/devices', device)
			.toPromise()
			.then((data) => plainToClass(Device, data))
			.catch(this.utils.handleResponseError);
	}
	...
	editDevice(device: Device): Promise<Device> {
		return this.http.patch('/walkoff/api/devices', device)
			.toPromise()
			.then((data) => plainToClass(Device, data))
			.catch(this.utils.handleResponseError);
	}
	...
	deleteDevice(deviceId: number): Promise<void> {
		return this.http.delete(`/walkoff/api/devices/${deviceId}`)
			.toPromise()
			.then(() => null)
			.catch(this.utils.handleResponseError);
	}
	...
	getDeviceApis(): Promise<AppApi[]> {
		return this.http.get('/walkoff/api/apps/apis?field_name=device_apis')
			.toPromise()
