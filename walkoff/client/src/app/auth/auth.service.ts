...
export class AuthService {
	...
	login(username: string, password: string): Promise<void> {
		return this.http.post('/walkoffapi/auth', { username, password })
			.toPromise()
	...
	logout(): Promise<void> {
		...
		return this.http.post('/walkoffapi/auth/logout', { refresh_token: sessionStorage.getItem(REFRESH_TOKEN_NAME) }, { headers })
			.toPromise()
			.then(() => {
				this.clearTokens();
			})
	...
	getAccessTokenRefreshed(): Promise<string> {
		...
		return this.http.post('/walkoffapi/auth/refresh', {}, { headers })
			.toPromise()
			.then((res: any) => {
