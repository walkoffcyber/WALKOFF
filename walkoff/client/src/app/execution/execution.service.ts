...
export class ExecutionService {
	 ...
	 */
	addWorkflowToQueue(workflow_id: string, execution_id: string, variables: EnvironmentVariable[] = []): Promise<WorkflowStatus> {
		...
		return this.http.post('/walkoff/api/workflowqueue', data)
			.toPromise()
	...
	performWorkflowStatusAction(workflowId: string, action: string): Promise<void> {
		return this.http.patch('/walkoff/api/workflowqueue', { execution_id: workflowId, status: action })
			.toPromise()
			.then(() => null)
			.catch(this.utils.handleResponseError);
	}
	...
	getWorkflowStatuses(page: number = 1): Promise<WorkflowStatus[]> {
		return this.http.get(`/walkoff/api/workflowqueue?page=${ page }`)
			.toPromise()
			.then((data: object[]) => plainToClass(WorkflowStatus, data))
			.catch(this.utils.handleResponseError);
	}
	...
	getWorkflowStatus(workflowExecutionId: string): Promise<WorkflowStatus> {
		return this.http.get(`/walkoff/api/workflowqueue/${workflowExecutionId}`)
			.toPromise()
			.then((data: object) => plainToClass(WorkflowStatus, data))
			.catch(this.utils.handleResponseError);
	}
